...
export function CatalogSpread({ rooms }: { rooms: Room[] }) {
  return (
    <div className="mx-auto grid max-w-[1380px] grid-cols-1 gap-5 px-5 pb-10 sm:grid-cols-2 sm:gap-px sm:px-8 lg:px-12">
      {rooms.map((room, index) => (
        <motion.article key={room.id} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.18 }} transition={{ duration: 0.45, delay: Math.min(index * 0.05, 0.2) }} className="page-shadow bg-paper p-5 sm:p-7 lg:p-9">

...
export function WhatsAppButton({ number, className = "" }: { number: string; className?: string }) {
  ...
  return (
    <button type="button" onClick={openWhatsApp} className={`focus-ring inline-flex min-h-12 items-center justify-center gap-3 rounded-full bg-[#1f6e4a] px-7 font-medium text-white shadow-lg shadow-emerald-900/10 transition hover:-translate-y-0.5 hover:bg-[#185c3d] ${className}`}>

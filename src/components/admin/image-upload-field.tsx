...
export function ImageUploadField({ name, label, target, initial = [], multiple = false }: { name: string; label: string; target: ImageTarget; initial?: string[]; multiple?: boolean }) {
  ...
  const showWatermarkToggle = target !== "logo";
  ...
  async function upload(files: FileList | null) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        next.push(await processRawImage(rawPath, target, watermark));
      }
  ...
  return (
    <div className="space-y-3">
      ...
      {showWatermarkToggle ? (
        <label className="flex w-fit items-center gap-2 rounded-lg border hairline bg-white px-3 py-2 text-sm">
          <input type="checkbox" checked={watermark} onChange={(event) => setWatermark(event.target.checked)} className="h-4 w-4 accent-[var(--accent-primary)]" />
          وضع العلامة المائية على الصور
        </label>
      ) : null}
